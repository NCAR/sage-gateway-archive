...
/**
 * Controller for form to allow adding an Award (award number) to a project.
 * <p/>
 * Edit will be a distinct operation with it's own controller.
 */
@Controller
@RequestMapping(value = {"project/{dataset}/awardeditor"})
public class ProjectAwardEditorController {

    private String formView = "projecteditor/awardeditor";

    private CadisDatasetService datasetService;
    private Validator validator;

    public ProjectAwardEditorController(Validator validator, CadisDatasetService datasetService) {
    ...
    public ModelAndView processSubmit(@ModelAttribute("command") NewAwardCommand command, BindingResult bindingResult, RedirectAttributes redirectAttributes) {
        ...
        if (bindingResult.hasErrors()) {
            ...
            resultView = "redirect:" + "/project/" + command.getDataset().getShortName() + "/awardNumber.html";
        }
